...
### cleansing startegy
# FIND missing > use 'complate.cases' , 'mean' for optimize and calculate mission %
# DROP missing row > use if your DF very very large
# REPLACE mission value with mean_with_no_na > berrter strategy
...
# calculate missing df (%)
mean(complete.cases(msleep))

# calculate complete df (%)
1 - mean(complete.cases(msleep))

# drop_na
clean_msleep <- drop_na(msleep)
mean(complete.cases(clean_msleep))

# replace na_with_mean
